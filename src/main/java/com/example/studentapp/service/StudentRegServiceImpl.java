...
public class StudentRegServiceImpl implements StudentRegService{
    ...
    public List<StudentRegistration> searchStudentDepertmentSemester(Semester theSemester, AcademicUnit dep) {
        return studentRegRepository.findStudentsBySemesterAndAcademicUnit(theSemester.getSemid(),dep.getId());
    }
    @Override
    public StudentRegistration searchStudentRegistration(StudentRegistration studentRegistration) {
        return studentRegRepository.findById(studentRegistration.getRegid()).get();
    }
}
